# Concept Map — Knowledge Graph of the Corpus

**Generated:** 42 concepts, 354 relationships from 25343 papers. Edges = co-occurrence (shared papers).

## Top concepts

| Concept | Kind | Papers |
|----------|------|--------|
| framework | curated | 6638 |
| Graph Neural Networks | taxonomy | 2984 |
| benchmark | curated | 2403 |
| survey | curated | 2308 |
| dataset | curated | 2213 |
| Knowledge Graphs | taxonomy | 1986 |
| Graph Security | taxonomy | 1973 |
| Network Science | taxonomy | 1586 |
| Graph Applications | taxonomy | 1580 |
| Graph Theory | taxonomy | 1562 |
| Graphrag | taxonomy | 1405 |
| Graph Algorithms | taxonomy | 1398 |
| Community Detection | taxonomy | 1393 |
| Graph Embeddings | taxonomy | 1290 |
| Temporal Graphs | taxonomy | 1134 |
| Graph Machine Learning | taxonomy | 1072 |
| Ontology | taxonomy | 1009 |
| Graph Construction | taxonomy | 952 |
| Graph Databases | taxonomy | 857 |
| Distributed Graphs | taxonomy | 848 |
| Graph Analytics | taxonomy | 726 |
| Graph Visualization | taxonomy | 679 |
| taxonomy | curated | 537 |
| Semantic Web | taxonomy | 499 |
| quantitative | curated | 440 |
| Graph Query Languages | taxonomy | 410 |
| case study | curated | 338 |
| experiment | curated | 298 |
| qualitative | curated | 275 |
| reproducibility | curated | 166 |
| systematic review | curated | 142 |
| literature review | curated | 138 |
| interview | curated | 34 |
| research gap | curated | 31 |
| mixed methods | curated | 20 |
| open access | curated | 19 |
| peer review | curated | 18 |
| triangulation | curated | 16 |
| grounded theory | curated | 1 |
| Survey | taxonomy | 0 |

## Relationships (top concepts → related concepts)

### framework

- benchmark — 933 shared paper(s)
- Graph Neural Networks — 859 shared paper(s)
- dataset — 744 shared paper(s)
- Graphrag — 676 shared paper(s)
- Graph Security — 642 shared paper(s)
- Knowledge Graphs — 607 shared paper(s)
- Graph Applications — 528 shared paper(s)
- Graph Embeddings — 381 shared paper(s)

### Graph Neural Networks

- framework — 859 shared paper(s)
- survey — 463 shared paper(s)
- benchmark — 327 shared paper(s)
- dataset — 261 shared paper(s)
- taxonomy — 83 shared paper(s)
- quantitative — 34 shared paper(s)
- case study — 31 shared paper(s)
- experiment — 28 shared paper(s)

### benchmark

- framework — 933 shared paper(s)
- dataset — 422 shared paper(s)
- Graph Neural Networks — 327 shared paper(s)
- Graph Analytics — 261 shared paper(s)
- Knowledge Graphs — 232 shared paper(s)
- Graphrag — 201 shared paper(s)
- Graph Applications — 200 shared paper(s)
- Graph Construction — 159 shared paper(s)

### survey

- Graph Neural Networks — 463 shared paper(s)
- Community Detection — 423 shared paper(s)
- Knowledge Graphs — 378 shared paper(s)
- framework — 330 shared paper(s)
- taxonomy — 248 shared paper(s)
- Graph Algorithms — 243 shared paper(s)
- Graph Visualization — 183 shared paper(s)
- Graph Databases — 182 shared paper(s)

### dataset

- framework — 744 shared paper(s)
- benchmark — 422 shared paper(s)
- Graph Security — 281 shared paper(s)
- Graph Neural Networks — 261 shared paper(s)
- Graph Applications — 260 shared paper(s)
- Knowledge Graphs — 208 shared paper(s)
- Graph Construction — 203 shared paper(s)
- Graphrag — 160 shared paper(s)

### Knowledge Graphs

- framework — 607 shared paper(s)
- survey — 378 shared paper(s)
- benchmark — 232 shared paper(s)
- dataset — 208 shared paper(s)
- taxonomy — 50 shared paper(s)
- quantitative — 40 shared paper(s)
- qualitative — 33 shared paper(s)
- case study — 29 shared paper(s)

### Graph Security

- framework — 642 shared paper(s)
- dataset — 281 shared paper(s)
- benchmark — 138 shared paper(s)
- case study — 34 shared paper(s)
- experiment — 30 shared paper(s)
- survey — 23 shared paper(s)
- quantitative — 23 shared paper(s)
- reproducibility — 16 shared paper(s)

### Network Science

- framework — 352 shared paper(s)
- dataset — 60 shared paper(s)
- benchmark — 49 shared paper(s)
- quantitative — 39 shared paper(s)
- survey — 32 shared paper(s)
- case study — 30 shared paper(s)
- qualitative — 16 shared paper(s)
- taxonomy — 10 shared paper(s)

### Graph Applications

- framework — 528 shared paper(s)
- dataset — 260 shared paper(s)
- benchmark — 200 shared paper(s)
- quantitative — 22 shared paper(s)
- case study — 19 shared paper(s)
- experiment — 17 shared paper(s)
- qualitative — 15 shared paper(s)
- survey — 15 shared paper(s)

### Graph Theory

- framework — 162 shared paper(s)
- benchmark — 21 shared paper(s)
- dataset — 20 shared paper(s)
- survey — 19 shared paper(s)
- quantitative — 18 shared paper(s)
- case study — 17 shared paper(s)
- experiment — 8 shared paper(s)
- triangulation — 7 shared paper(s)

### Graphrag

- framework — 676 shared paper(s)
- benchmark — 201 shared paper(s)
- dataset — 160 shared paper(s)
- case study — 23 shared paper(s)
- survey — 19 shared paper(s)
- taxonomy — 15 shared paper(s)
- quantitative — 14 shared paper(s)
- qualitative — 13 shared paper(s)

### Graph Algorithms

- survey — 243 shared paper(s)
- framework — 189 shared paper(s)
- dataset — 38 shared paper(s)
- benchmark — 37 shared paper(s)
- taxonomy — 20 shared paper(s)
- quantitative — 18 shared paper(s)
- experiment — 13 shared paper(s)
- case study — 13 shared paper(s)

### Community Detection

- survey — 423 shared paper(s)
- framework — 231 shared paper(s)
- benchmark — 90 shared paper(s)
- dataset — 78 shared paper(s)
- quantitative — 38 shared paper(s)
- taxonomy — 27 shared paper(s)
- qualitative — 15 shared paper(s)
- experiment — 13 shared paper(s)

### Graph Embeddings

- framework — 381 shared paper(s)
- survey — 171 shared paper(s)
- benchmark — 146 shared paper(s)
- dataset — 90 shared paper(s)
- taxonomy — 25 shared paper(s)
- qualitative — 23 shared paper(s)
- quantitative — 22 shared paper(s)
- experiment — 20 shared paper(s)

### Temporal Graphs

- framework — 287 shared paper(s)
- benchmark — 104 shared paper(s)
- dataset — 102 shared paper(s)
- quantitative — 12 shared paper(s)
- experiment — 12 shared paper(s)
- taxonomy — 9 shared paper(s)
- survey — 9 shared paper(s)
- case study — 9 shared paper(s)

### Graph Machine Learning

- framework — 369 shared paper(s)
- benchmark — 147 shared paper(s)
- dataset — 77 shared paper(s)
- survey — 43 shared paper(s)
- experiment — 14 shared paper(s)
- taxonomy — 14 shared paper(s)
- quantitative — 9 shared paper(s)
- reproducibility — 7 shared paper(s)

### Ontology

- framework — 305 shared paper(s)
- taxonomy — 205 shared paper(s)
- benchmark — 134 shared paper(s)
- dataset — 115 shared paper(s)
- survey — 51 shared paper(s)
- quantitative — 20 shared paper(s)
- experiment — 19 shared paper(s)
- case study — 19 shared paper(s)

### Graph Construction

- framework — 232 shared paper(s)
- dataset — 203 shared paper(s)
- benchmark — 159 shared paper(s)
- experiment — 26 shared paper(s)
- survey — 19 shared paper(s)
- quantitative — 14 shared paper(s)
- qualitative — 14 shared paper(s)
- case study — 13 shared paper(s)

### Graph Databases

- survey — 182 shared paper(s)
- framework — 156 shared paper(s)
- dataset — 66 shared paper(s)
- benchmark — 59 shared paper(s)
- quantitative — 36 shared paper(s)
- qualitative — 24 shared paper(s)
- taxonomy — 16 shared paper(s)
- case study — 15 shared paper(s)

### Distributed Graphs

- framework — 185 shared paper(s)
- benchmark — 37 shared paper(s)
- dataset — 36 shared paper(s)
- case study — 13 shared paper(s)
- quantitative — 5 shared paper(s)
- survey — 5 shared paper(s)
- experiment — 5 shared paper(s)

### Graph Analytics

- framework — 273 shared paper(s)
- benchmark — 261 shared paper(s)
- dataset — 87 shared paper(s)
- quantitative — 21 shared paper(s)
- reproducibility — 15 shared paper(s)
- case study — 12 shared paper(s)
- qualitative — 11 shared paper(s)
- experiment — 11 shared paper(s)

### Graph Visualization

- survey — 183 shared paper(s)
- framework — 67 shared paper(s)
- quantitative — 50 shared paper(s)
- qualitative — 40 shared paper(s)
- dataset — 30 shared paper(s)
- case study — 16 shared paper(s)
- experiment — 14 shared paper(s)
- interview — 12 shared paper(s)

### taxonomy

- survey — 248 shared paper(s)
- Ontology — 205 shared paper(s)
- framework — 153 shared paper(s)
- benchmark — 110 shared paper(s)
- dataset — 85 shared paper(s)
- Graph Neural Networks — 83 shared paper(s)
- Knowledge Graphs — 50 shared paper(s)
- systematic review — 27 shared paper(s)

### Semantic Web

- framework — 88 shared paper(s)
- dataset — 26 shared paper(s)
- benchmark — 19 shared paper(s)
- survey — 11 shared paper(s)
- case study — 8 shared paper(s)
- experiment — 6 shared paper(s)
- reproducibility — 5 shared paper(s)
- literature review — 4 shared paper(s)

### quantitative

- framework — 153 shared paper(s)
- survey — 133 shared paper(s)
- qualitative — 126 shared paper(s)
- dataset — 52 shared paper(s)
- Graph Visualization — 50 shared paper(s)
- Knowledge Graphs — 40 shared paper(s)
- Network Science — 39 shared paper(s)
- Community Detection — 38 shared paper(s)


Generated by `tools/relate_concepts.py` — do not edit by hand.